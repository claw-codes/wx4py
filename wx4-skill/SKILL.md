...
#### 监听多个群聊消息
```python
from wx4py import MessageEvent, WeChatClient
...
with WeChatClient(auto_connect=True) as wx:
    wx.listen_groups(groups, on_message, block=True)
```
...
with WeChatClient(auto_connect=True) as wx:
    wx.auto_reply_groups(["工作群"], custom_reply, block=True, reply_on_at=True)
```
...
| 监听多个群聊 | `listen_groups(groups, on_message, block=True)` | `wx.listen_groups(["群1", "群2"], on_message, block=True)` |
| 自动回复群聊 | `auto_reply_groups(groups, reply_func, reply_on_at=True)` | `wx.auto_reply_groups(["群1"], reply, block=True, reply_on_at=True)` |
| AI 自动回复 | `AIResponder(ai_client, context_size=8)` | `AIResponder(ai, context_size=8, reply_on_at=True)` |
...
A: 可以。`auto_reply_groups()` 接收任意函数作为回复回调，函数返回字符串就会入队发送，返回空字符串或 `None` 就不会回复。
